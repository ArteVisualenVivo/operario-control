...
function getFirebaseAdmin() {
    ...
    if (!fs.existsSync(serviceAccountPath)) {
        throw new Error("[FIREBASE] Missing sync-agent/service-account.json. " +
    ...
    if (apps.length > 0) {
        for (const app of apps) {
            try {
                ...
                console.warn("[FIREBASE] Failed to delete existing Firebase app:", deleteError instanceof Error ? deleteError.message : deleteError);
            }
        }
    }
    admin.initializeApp({
...
export async function syncItems(items, options) {
    ...
    for (const item of items) {
        ...
        if (item.stockWarning) {
            result.warnings.push(`Stock negativo para "${item.name}" (código: ${item.codigo}): ${item.stockTotal}`);
        }
        if (match) {
            ...
            result.skipped++;
            result.warnings.push(`Material no encontrado en Firestore: "${item.name}" omitido (strictMode)`);
        }
        counter++;
...
export async function syncRepairsToMaintenance(buffer) {
    const admin = getFirebaseAdmin();
    const { getFirestore } = require("firebase-admin/firestore");
    const db = getFirestore();
    const BATCH_LIMIT = 400;
    ...
    const findHeaderRowIndex = () => {
        return rows.findIndex((row) => {
            ...
            const hasOrder = normalizedCells.some((cell) => ["numero", "nro", "nro_orden"].includes(cell) || cell.includes("numero"));
            const hasDate = normalizedCells.some((cell) => cell.startsWith("fecha") || cell.includes("entrega") || cell.includes("egreso"));
    ...
    const logSkippedRow = (rowNumber, reason, details) => {
        result.skipped++;
        result.warnings.push(`Fila ${rowNumber} omitida: ${reason}`);
        console.warn(`[MAINTENANCE ROW] skip row ${rowNumber}: ${reason}`, details);
    };
    const isValidDateObject = (date) => date instanceof Date && Number.isFinite(date.getTime());
    const buildCheckedDate = (year, month, day, hours = 0, minutes = 0, seconds = 0) => {
    ...
    const isValidOrderNumber = (value) => {
        ...
        // 🔧 FIX REAL (solo regex)
        return /^x\s?\d{3,6}-\d{4,10}$/i.test(value.replace(/\s+/g, " ").trim());
    };
    const cleanOptionalText = (value) => {
        const text = String(value ?? "").trim();
        if (!text)
            return null;
        if (HEADER_BLACKLIST.includes(normalizeToken(text)))
            return null;
        return text;
    };
    ...
    const isHeaderRow = (row, orderNumber, entryDateRaw) => {
    ...
    const parseEntryDate = (value) => {
        if (value instanceof Date) {
            if (!isValidDateObject(value))
                return null;
            return buildCheckedDate(value.getFullYear(), value.getMonth() + 1, value.getDate(), value.getHours(), value.getMinutes(), value.getSeconds());
        }
        if (typeof value === "number" && XLSX.SSF && typeof XLSX.SSF.parse_date_code === "function") {
            const parsed = XLSX.SSF.parse_date_code(value);
            if (parsed && parsed.y) {
                return buildCheckedDate(parsed.y, parsed.m ?? 1, parsed.d ?? 1, parsed.H ?? 0, parsed.M ?? 0, parsed.S ?? 0);
            }
            return null;
        }
        ...
        if (ddmmyyyy) {
            ...
            if (year < 100)
                year += 2000;
            return buildCheckedDate(year, Number(ddmmyyyy[2]), Number(ddmmyyyy[1]), Number(ddmmyyyy[4] ?? 0), Number(ddmmyyyy[5] ?? 0), Number(ddmmyyyy[6] ?? 0));
        }
        const iso = normalized.match(/^(\d{4})-(\d{1,2})-(\d{1,2})(?:[T\s](\d{1,2}):(\d{2})(?::(\d{2}))?)?/);
        if (iso) {
            return buildCheckedDate(Number(iso[1]), Number(iso[2]), Number(iso[3]), Number(iso[4] ?? 0), Number(iso[5] ?? 0), Number(iso[6] ?? 0));
        }
        return null;
    };
    const commitBatch = async (lastBatchPayload) => {
        if (counter === 0)
            return;
